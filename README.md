...
A janela é 1200 por 700 (menor quando a área de trabalho não comporta; issue #20). O visual é a direção E ("Pergaminho") escolhida em 2026-09-07: bege `#ebe7dc` em toda a janela, sem véu nem cartão sobre a arte, e a deusa Gabija (`tos-1` do site) de pé na borda direita, cortada embaixo por uma máscara suave e com uma poça de luz quente atrás (`Backdrop`). Ela se mexe: `assets/goddess.webm` é a animação de repouso (cabelo, chama, um piscar) em VP9 com canal alfa, 1440 por 1440 px, 24 fps, sem som, 1,6 MB, 4 s em laço; `assets/goddess.webp` é o primeiro quadro, mostrado até o vídeo carregar e no lugar dele quando o sistema pede menos movimento (`prefers-reduced-motion`). O vídeo pausa com a janela escondida. Os dois saem de um clipe do Kling (imagem para vídeo, a `tos-1` sobre fundo verde) já recortado fora do repositório. O recorte deixou verde perto da chama, uma névoa entre as volutas e, duas vezes, uma voluta inteira que se apaga; `tools/flame-goddess.py` pinta esse verde com as cores da chama, não mexe em mais nada e documenta os comandos do ffmpeg; `tools/loop-goddess.py` fecha o laço, porque o último quadro do Kling não volta ao primeiro (corta no quadro 95, que quase repete o primeiro, e funde os dois ao longo do fluxo óptico para nada dobrar). Nas telas Notícias e Configurações ela dá um passo para a direita (só `transform`), para a coluna mais larga ter espaço. O `App.tsx` do renderer monta:
...
`SettingsView` (`src/renderer/src/components/SettingsView.tsx`): uma tela como Início e Notícias, não um diálogo por cima delas. O título Configurações com as seções **Jogo** e **Launcher** como links de texto ao lado, e embaixo uma linha por configuração que rola na coluna de 600 px (título, uma linha de explicação, o controle à direita). Jogo: pasta do jogo, jogar sem conexão, a correção AMD, Reparar, Verificar runtimes. Os argumentos de inicialização (`launchArgs`) e o que fazer ao iniciar o jogo (`afterLaunch`, fechar o launcher por padrão) continuam no `config.json`, mas não aparecem na tela: o jogador não tem por que mexer neles. Launcher: idioma, conexões de download, aceleração de hardware, a versão com Verificar agora e a pasta de logs. As chaves ligado/desligado são o componente `Toggle`; cada controle salva na hora, não há Aplicar, e nenhuma configuração mudou de comportamento. Sai-se pela navegação, pela engrenagem, com Esc, ou por Reparar e Verificar (que voltam ao Jogar); sair encerra a visita, e a próxima abre na seção inicial sem nada da anterior. `?view=settings` abre na seção Jogo e `?view=settings:launcher` na Launcher, a mesma string que `YUFA_VIEW` repassa; os ajudantes puros ficam em `src/renderer/src/lib/settingsDialog.ts`, com testes.
