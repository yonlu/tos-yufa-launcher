...
A janela é 1200 por 700 (menor quando a área de trabalho não comporta; issue #20). O visual é a direção E ("Pergaminho") escolhida em 2026-09-07: bege `#ebe7dc` em toda a janela, sem véu nem cartão sobre a arte, e a deusa Gabija (`tos-1` do site) de pé na borda direita, cortada embaixo por uma máscara suave e com uma poça de luz quente atrás (`Backdrop`). Ela se mexe: `assets/goddess.webm` é a animação de repouso (cabelo, chama, um piscar) em VP9 com canal alfa, 1440 por 1440 px, 24 fps, sem som, 2,1 MB, 5 s em laço; `assets/goddess.webp` é o primeiro quadro, mostrado até o vídeo carregar e no lugar dele quando o sistema pede menos movimento (`prefers-reduced-motion`). O vídeo pausa com a janela escondida. Os dois saem de um clipe do Kling (imagem para vídeo, a `tos-1` sobre fundo verde) já recortado fora do repositório. O recorte deixou verde perto da chama, uma névoa entre as volutas e, duas vezes, uma voluta inteira que se apaga; `tools/flame-goddess.py` pinta esse verde com as cores da chama, não mexe em mais nada e documenta os comandos do ffmpeg. Nas telas Notícias e Configurações ela dá um passo para a direita (só `transform`), para a coluna mais larga ter espaço. O `App.tsx` do renderer monta:
